// Level: Circular Maze
// Strategy: run A* on a padded grid, smooth the route with line-of-sight
// checks, and follow it slowly through the circular passages.
function dist(a, b) {
  var x = a.x - b.x,
    y = a.y - b.y;
...
function clearLine(a, b, rocks) {
  // Keep 55 units of padding around each obstacle.
  var vx = b.x - a.x,
    vy = b.y - a.y,
    ll = vx * vx + vy * vy;
  for (var i = 0; i < rocks.length; i++) {
    ...
    var x = a.x + t * vx - o.pos.x,
      y = a.y + t * vy - o.pos.y;
    if (x * x + y * y < (o.radius + 55) * (o.radius + 55)) return false;
  }
  return true;
}
function plan(start, goal, rocks) {
  ...
  function blocked(p) {
    for (var j = 0; j < rocks.length; j++)
      if (dist(p, rocks[j].pos) < rocks[j].radius + 55) return true;
    return false;
  }
  function nearest(p) {
  ...
  // Reconstruct the route and collapse every visible run into one segment.
  var raw = [goal],
    c = g,
    guard = 0;
...
function fly(q, last) {
  ...
  var speed = Math.min(1.4, Math.sqrt(d) * 0.28),
    ax = (dx / (d || 1)) * speed - v.x,
    ay = (dy / (d || 1)) * speed - v.y;
  var angle = Math.atan2(ay, ax),
    e = wrap(p.angular - angle),
