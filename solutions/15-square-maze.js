...
// checks, and follow it through the square passages.
function dist(a, b) {
  var x = a.x - b.x,
    y = a.y - b.y;
...
function clearLine(a, b, rocks) {
  // Keep the wider 80-unit margin needed by the square maze.
  var vx = b.x - a.x,
    vy = b.y - a.y,
    ll = vx * vx + vy * vy;
  for (var i = 0; i < rocks.length; i++) {
    ...
    var x = a.x + t * vx - o.pos.x,
      y = a.y + t * vy - o.pos.y;
    if (x * x + y * y < (o.radius + 80) * (o.radius + 80)) return false;
  }
  return true;
}
function plan(start, goal, rocks) {
  ...
  function blocked(p) {
    for (var j = 0; j < rocks.length; j++)
      if (dist(p, rocks[j].pos) < rocks[j].radius + 80) return true;
    return false;
  }
  function nearest(p) {
...
function fly(q, last) {
  ...
  var speed = Math.min(3, Math.sqrt(d) * 0.35),
    ax = (dx / (d || 1)) * speed - v.x,
    ay = (dy / (d || 1)) * speed - v.y;
